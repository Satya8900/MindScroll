...
const signupUser = asyncHandler(
    async (req, res) => {
        ...
        if (!name || !email || !number || !password) {
            return res.status(400).json(new ApiError(400, "All required fields must be filled correctly.", "Validation Error"))
        }
        ...
        if (check_available_result.rowCount > 0) {
            return res.status(400).json(new ApiError(400, "User account already exists. Please log in.", "Account Exist"))
        }
        else {
            ...
            res.status(201).json(new ApiResponse(201, [signup_insert_result.rowCount], "Signed Up Successfully.."));
        }
    }
);
